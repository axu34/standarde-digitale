...
const links = [
  ...
  { href: "/metodologie", label: "Cum notăm" },
];
...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-black/10 bg-white">
      <div className="mx-auto flex h-[72px] max-w-[1080px] items-center justify-between px-4 lg:px-8">
        <Link href="/" className="flex items-center gap-3" aria-label="Standarde digitale — acasă">
          <img
            src="/DACIA_K.svg"
            alt="Dacia"
            className="h-8 w-auto border-r border-black pr-3"
          />
          <span className="font-block text-sm font-bold uppercase tracking-wide text-black">
            Standarde digitale
          </span>
        </Link>
        ...
        <nav className="hidden items-center gap-10 lg:flex" aria-label="Principal">
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="font-block text-sm font-bold uppercase tracking-wide text-ink"
          >
            {BRAND.agency}
          </a>
        </nav>
